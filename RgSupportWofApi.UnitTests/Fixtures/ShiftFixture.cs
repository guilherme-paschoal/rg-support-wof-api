...
namespace RgSupportWofApi.UnitTests.Fixtures
{
    public class ShiftFixture
    {
        ...
        public IList<Shift> GetShiftsForToday()
        {
            var engineers = new EngineerFixture().GetNewListOfEnginers();
            var shifts = new List<Shift>
            {
                new Shift { Date = DateFixture.Today, ShiftOrder = 1, Engineer = engineers[0] },
                new Shift { Date = DateFixture.Today, ShiftOrder = 2, Engineer = engineers[1] }
            };
            return shifts;
        }
    }
